...
function getAverage(val1, val2) {
  if (!Number.isFinite(val1) || !Number.isFinite(val2)) {
    throw new Error('Cannot calculate average for infinite values.');
  }
  return (val1 + val2) / 2;
}
...
function getLastDigit(value) {
  const result = value.toString().slice(-1);
  return result;
}
getLastDigit(67554);
...
function getSumOfNumbers(x1, x2, x3) {
  ...
  if (Math.abs(result - 0.6) < tolerance) {
    return 0.6;
  }
  return result;
}

getSumOfNumbers(0.11, 0.2, 3);
/**
...
function getCountOfOddNumbers(number) {
  if (Number.isInteger(number) && number >= 0) {
    const res = Math.floor((number + 1) / 2);
    return res;
  }
  return false;
}
